...
export async function POST(request: NextRequest) {
  try {
    ...
    // 1. R4_QUESTION_SHOW: đang hiển thị câu hỏi (có thể timer đang chạy hoặc đã hết)
    // 2. R4_QUESTION_LOCK_MAIN: đã khóa đáp án đội đang thi
    // 3. R4_JUDGE_MAIN: đang trong quá trình chấm
    ...
    if (gameState.phase === "R4_STAR_CONFIRMATION") {
      // Không cho phép chấm khi đang chờ đội xác nhận ngôi sao
      console.log("[Round4 Star] Error: Cannot judge during star confirmation phase");
      return NextResponse.json(
        { 
          error: "Không thể chấm điểm. Vui lòng đợi đội xác nhận Ngôi sao hy vọng trước khi chấm." 
        },
        { status: 400 }
      );
      ...
      return NextResponse.json(
        { 
          error: `Không thể chấm đáp án đội đang thi ở phase hiện tại: ${gameState.phase}. Chỉ cho phép chấm khi phase là R4_QUESTION_SHOW, R4_QUESTION_LOCK_MAIN, hoặc R4_JUDGE_MAIN.` 
        },
        { status: 400 }
      );
    }
